...
def experiment_parallel(nums, n, backend):
    ...
    for j in range(iterations_per_marked_item_set):
        ...
        result = execute(qc, backend, shots=shots, memory=True).result() # Should be replaced with "transpile" and "backend.run" or something like that, since execute is deprecated
